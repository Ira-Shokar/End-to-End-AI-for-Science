...
def render_tool_result(name: str, result: str, max_len: int = 1200) -> str:
    preview = html_escape(result[:max_len] + ("..." if len(result) > max_len else ""))
...
class ContentResourceServer:
    ...
    def _rebuild_index(self):
        ...
        for category, directory, extensions in categories:
            ...
            for fp in directory.iterdir():
                if not fp.is_file() or fp.suffix not in extensions or fp.name.startswith('.'):
                    continue
                try:
    ...
    def _list_files(self, args: Dict) -> Dict:
        ...
        files = [info for info in sorted(self._file_index.values(), key=lambda x: x["name"]) 
    ...
    def _extract_code(self, args: Dict) -> Dict:
        paths, lang_filter = args.get("paths", []), args.get("language", "").lower()
        blocks = []
    ...
    def list_all_sources(self) -> List[Tuple[str, str]]:
        return [(info["name"], path) for path, info in sorted(self._file_index.items(), key=lambda x: (x[1]["category"], x[1]["name"]))]
    ...
    for iteration in range(MAX_ITERATIONS):
        ...
        for chunk in llm.stream(messages, tools=tools):
            if chunk["type"] == "error":
                ...
                thinking_buf += chunk.get("content", "")
    ...
    if "Generate" in ops and recipe.strip():
        ...
        for chunk in llm.stream([{"role": "user", "content": f"{recipe}\n\n---\n\nCONTENT:\n{combined}"}]):
            if chunk.get("type") == "content":
                ...
                history[-1]["content"] = status + plan.render() + "\n\n---\n\n" + output + format_thinking_tags(generated)
                yield history
...
def do_save(history: List[Dict], filename: str) -> str:
    ...
    content = next((m.get("content", "") for m in reversed(history) if m.get("role") == "assistant"), "")
    print(content)
...
def build_interface() -> gr.Blocks:
    ...
    with gr.Blocks(title="DLI Content Processor") as app:
        ...
        def stop_chat(history):
            h = list(history or [])
            if h and h[-1].get("role") == "assistant" and "status-line" not in h[-1].get("content", ""):
