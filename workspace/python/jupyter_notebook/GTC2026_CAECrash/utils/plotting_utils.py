...
def plot_l2_errors(predictions, ground_truths, save_path='l2_error_plot.png'):
    ...
    for sample_idx in range(num_samples):
        ...
        for t in range(T):
            # Compute L2 error at timestep t
            error = np.linalg.norm(pred[t] - gt[t], axis=1)  # [N]
            l2_error = np.mean(error)
            
            # Compute relative error
            gt_norm = np.linalg.norm(gt[t], axis=1)  # [N]
    ...
    for sample_idx in range(all_l2_errors.shape[0]):
        axes[0].plot(timesteps, all_l2_errors[sample_idx], 
                    color='gray', alpha=0.3, linewidth=1)
    axes[0].plot(timesteps, mean_l2, 'o-', color='red', 
                linewidth=2.5, markersize=6, label='Mean', zorder=10)
    axes[0].fill_between(timesteps, mean_l2 - std_l2, mean_l2 + std_l2, 
    ...
    for sample_idx in range(all_relative_errors.shape[0]):
        axes[1].plot(timesteps, all_relative_errors[sample_idx], 
                    color='gray', alpha=0.3, linewidth=1)
    axes[1].plot(timesteps, mean_rel, 's-', color='red', 
                linewidth=2.5, markersize=6, label='Mean', zorder=10)
    axes[1].fill_between(timesteps, mean_rel - std_rel, mean_rel + std_rel, 
                         color='red', alpha=0.2, label='±1 std')
...
def load_vtp_data(vtp_file_path: str):
    ...
    Returns:
        Dictionary with keys: mesh, points, x, y, z, point_data, field_names,
        disp_keys, first_field, last_field, thick_vals, pstrain_last,
        stress_last, bc_keys, field_data.
    """
    import pyvista as pv

    # Read VTP file using pyvista (no physicsnemo dependency)
    mesh = pv.read(vtp_file_path)
    points = mesh.points
    x, y, z = points[:, 0], points[:, 1], points[:, 2]

    # Get all point data field names
    field_names = list(mesh.point_data.keys())

    # Displacement fields (temporal)
    disp_keys = [k for k in field_names if k.startswith("displacement_t")]
    disp_keys.sort(key=lambda k: float(k.split('_t')[-1]))
    first_field = disp_keys[0] if disp_keys else None
    last_field = disp_keys[-1] if disp_keys else None

    # Thickness (static)
    thick_vals = mesh.point_data.get("thickness", None)

    # Plastic strain
    pstrain_last = mesh.point_data.get("plastic_strain", None)

    # Von Mises stress
    stress_last = mesh.point_data.get("von_mises_stress", None)

    # Boundary conditions (field data)
    bc_keys = [k for k in mesh.field_data.keys() if k.startswith("bc_")]
    ...
    return {
        ...
        "thick_vals": thick_vals,
        "pstrain_last": pstrain_last,
...
def save_trajectory_to_vtp(positions_with_physics, output_dir, run_id):
    ...
    # Use the first frame as the base geometry
    data_0 = positions_with_physics[0].detach().cpu().numpy()
    mesh = pv.PolyData(np.ascontiguousarray(data_0[:, :3]))

    for t in range(T):
        data_t = positions_with_physics[t].detach().cpu().numpy()
        # Calculate displacement relative to t=0
        disp = data_t[:, :3] - data_0[:, :3]
        mesh.point_data[f"displacement_t{t:03d}"] = np.ascontiguousarray(disp)
        
        if C >= 4:
            mesh.point_data[f"plastic_strain_t{t:03d}"] = np.ascontiguousarray(data_t[:, 3])
        if C >= 5:
            mesh.point_data[f"von_mises_stress_t{t:03d}"] = np.ascontiguousarray(data_t[:, 4])

    mesh.save(os.path.join(output_dir, f"run_{run_id:03d}.vtp"))

def print_vtp_summary(vtp_data: dict):
    """
    Print a formatted summary of VTP data contents.

    Args:
        vtp_data: Dictionary returned by :func:`load_vtp_data`.
    """
    ...
    print(f"⏱️  Timesteps:  {len(disp_keys)} (from {first_field} to {last_field})")
    print(f"📊 Fields:     {len(field_names)} total")
    print(f"\n--- Available Data ---")
    print(f"  ✅ Displacement:    {len(disp_keys)} timesteps")
    print(
        f"  {'✅' if thick_vals is not None else '⚠️ '} Thickness:       "
        f"{'Available' if thick_vals is not None else 'Not found'}"
    )
    print(
        f"  {'✅' if pstrain_last is not None else '⚠️ '} Plastic Strain:  "
        f"{'Available' if pstrain_last is not None else 'Not in VTP (regenerate with updated ETL)'}"
    )
    print(
        f"  {'✅' if stress_last is not None else '⚠️ '} Von Mises Stress: "
        f"{'Available' if stress_last is not None else 'Not in VTP (regenerate with updated ETL)'}"
    )
    if bc_keys:
        print(f"  Boundary Conds:  {len(bc_keys)} keys")
    else:
        print("   Boundary Conds:  Not found")
...
def plot_vtp_multifield(vtp_data: dict, subsample_step: int = 1):
    """
    Generate a multi-panel 3D scatter visualization from VTP data.

    Plots displacement (start and end), thickness, plastic strain, and
    von Mises stress where available.

    Args:
        vtp_data: Dictionary returned by :func:`load_vtp_data`.
        subsample_step: Sub-sampling stride for faster rendering.
    """
    from vtk.util import numpy_support
    ...
    def get_warped_data(field_name):
        """Get deformed coordinates and displacement magnitude."""
    ...
    num_plots = 3 + (1 if pstrain_last is not None else 0) + (1 if stress_last is not None else 0)
    ncols, nrows = 3, (num_plots + 2) // 3
    fig = plt.figure(figsize=(16, 5.5 * nrows))
    step = subsample_step
    plot_idx = 1

    # Plot 1: Start Displacement
    ...
    if thick_vals is not None:
        ...
        p3 = ax3.scatter(x[::step], y[::step], z[::step], c=thick_vals[::step], cmap='viridis', s=3, alpha=0.8)
        setup_axis(ax3, "Shell Thickness\n(Static Property)")
        fig.colorbar(p3, ax=ax3, shrink=0.6, label='Thickness (mm)', pad=0.1)
        plot_idx += 1

    # Plot 4: Plastic Strain (if available)
    if pstrain_last is not None:
        ax4 = fig.add_subplot(nrows, ncols, plot_idx, projection='3d')
        p4 = ax4.scatter(x1[::step], y1[::step], z1[::step], c=pstrain_last[::step], cmap='plasma', s=3, vmin=0, alpha=0.8)
    ...
    # Plot 5: Von Mises Stress (if available)
    if stress_last is not None:
        ax5 = fig.add_subplot(nrows, ncols, plot_idx, projection='3d')
        p5 = ax5.scatter(x1[::step], y1[::step], z1[::step], c=stress_last[::step], cmap='hot', s=3, vmin=0, alpha=0.8)
