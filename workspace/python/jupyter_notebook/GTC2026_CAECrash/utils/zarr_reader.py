...
def load_zarr_store(zarr_path: str):
    """
    Load mesh positions, edges, and numeric point data fields from a Zarr store.

    Non-numeric or metadata fields are skipped gracefully.
    """
    ...
    mesh_pos = np.asarray(store["mesh_pos"], dtype=np.float64)

    if "edges" not in store:
        raise KeyError(f"'edges' not found in Zarr store {zarr_path}")
    edges = np.asarray(store["edges"], dtype=np.int64)
    ...
    for name in store.keys():
        ...
        try:
            ...
            # Require array-like numeric data
            ...
            # Skip object / string / structured dtypes
            if not np.issubdtype(data.dtype, np.number):
                continue

            point_data_dict[name] = data.astype(np.float32)

        except (ValueError, TypeError):
            # Gracefully skip non-numeric or malformed fields
            continue
...
def process_zarr_data(
    ...
    Includes logic to handle transposed temporal features (e.g. stress/strain).
    """
    ...
    for zarr_path in zarr_stores:
        ...
        try:
            ...
            # Validate and Fix point data features
            for name, data in point_data_dict.items():
                if data.ndim == 1:
                    ...
                    # --- FIX: Handle (Time, Nodes) to (Nodes, Time) transposition ---
                    if data.shape[0] != num_nodes and data.shape[1] == num_nodes:
                        if logger:
                            logger.info(f"Transposing temporal field '{name}' from {data.shape} to ({num_nodes}, {data.shape[0]})")
                        data = data.T
...
class Reader:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # Load global features from JSON if provided
        global_features = None
        if global_features_filepath:
            import json
            from pathlib import Path
            
            gf_path = Path(global_features_filepath)
            if gf_path.exists():
                ...
                for i in range(num_samples):
                    key = f"run{i+1}"
                    if key not in gf_data:
                        key = f"Run{i+1}"
                    if key not in gf_data:
                        key = str(i)
                    
                    if key in gf_data:
                        global_features.append(gf_data[key])
                    else:
                        if logger:
                            logger.warning(f"No global features found for sample {i}")
                        global_features.append({})
                
                if logger:
                    logger.info(f"Loaded global features from {global_features_filepath}")
            else:
                if logger:
                    logger.warning(f"Global features file not found: {global_features_filepath}")
        
        return srcs, dsts, point_data, global_features